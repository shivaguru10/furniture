...
import logoUrl from './assets/logo-furniture.svg';
...
const defaultImageFallback = FALLBACK_IMAGE_URL || 'https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?q=80&w=1200&auto=format&fit=crop';
...
const clearProductStructuredData = () => {
  document.getElementById('product-jsonld')?.remove();
};

const setProductStructuredData = (product, image, price) => {
  ...
  script.textContent = JSON.stringify({
    ...
    name: product.name,
    image,
    description: product.description || `${product.name} from ${product.brand || 'Gugan Furniture'}`,
    brand: {
      '@type': 'Brand',
      name: product.brand || 'Gugan Furniture',
    },
...
const CATEGORY_DEFINITIONS = [
  { name: 'Living Room', image: 'https://images.unsplash.com/photo-1555041469-a586c61ea9bc?q=80&w=600&auto=format&fit=crop' },
  { name: 'Bedroom', image: 'https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?q=80&w=600&auto=format&fit=crop' },
  { name: 'Dining', image: 'https://images.unsplash.com/photo-1617806118233-18e1de247200?q=80&w=600&auto=format&fit=crop' },
  { name: 'Office', image: 'https://images.unsplash.com/photo-1518455027359-f3f8164ba6bd?q=80&w=600&auto=format&fit=crop' },
  { name: 'Outdoor', image: 'https://images.unsplash.com/photo-1600210492493-0946911123ea?q=80&w=600&auto=format&fit=crop' },
  { name: 'Storage', image: 'https://images.unsplash.com/photo-1616627781431-23b776aad6c6?q=80&w=600&auto=format&fit=crop' },
  { name: 'Decor', image: 'https://images.unsplash.com/photo-1513519245088-0e12902e5a38?q=80&w=600&auto=format&fit=crop' },
  { name: 'Lighting', image: 'https://images.unsplash.com/photo-1513506003901-1e6a229e2d15?q=80&w=600&auto=format&fit=crop' },
];

const CATEGORY_NAMES = CATEGORY_DEFINITIONS.map((category) => category.name);
const DESKTOP_NAV_CATEGORIES = ['Living Room', 'Bedroom', 'Dining', 'Office', 'Decor'];
...
const categoryDomId = (category) => String(category).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
...
const normalizeCategoryName = (name) => {
  ...
  const alias = CATEGORY_ALIASES[rawName.toLowerCase()];
  if (Object.prototype.hasOwnProperty.call(CATEGORY_ALIASES, rawName.toLowerCase())) return alias;
  return CATEGORY_DEFINITIONS.find((category) => category.name.toLowerCase() === rawName.toLowerCase())?.name || rawName;
};
const furnitureHeroText = (value, fallback) => {
  const text = String(value || '').trim();
  if (!text) return fallback;
  if (/casual|wear|fashion|summer collection/i.test(text)) return fallback;
  return text;
};
const isLegacyHeroImage = (value) => {
  const text = String(value || '').toLowerCase();
  return text.includes('hero.png') || text.includes('1523381210434') || text.includes('1445205170230');
};
const CATEGORY_IMAGE_FALLBACKS = {
  ...Object.fromEntries(CATEGORY_DEFINITIONS.map((category) => [category.name.toUpperCase(), category.image])),
  HOME: 'https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?q=80&w=600&auto=format&fit=crop',
};
...
const DEFAULT_SHOWCASE_CATEGORIES = ['Living Room', 'Bedroom', 'Dining', 'Office', 'Outdoor', 'Storage', 'Decor', 'Lighting'];
...
const DEFAULT_HOME_VIDEO_POSTER = 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?q=80&w=1600&auto=format&fit=crop';
const TESTIMONIALS = [
  { name: 'Syam Kumar', role: 'Interior Stylist', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&h=150&fit=crop&crop=faces', text: "The furniture collection at Gugan Furniture is truly unique. I love how they blend practical comfort with modern luxury. The finish and build quality are worth every rupee." },
  { name: 'shivaguru', role: 'Verified Buyer', avatar: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=150&h=150&fit=crop&crop=faces', text: "I ordered a sofa set and a coffee table for our living room. Delivery was incredibly fast, the packaging felt premium, and the pieces changed the whole room." },
  { name: 'tilak', role: 'Creative Director', avatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=150&h=150&fit=crop&crop=faces', text: "As someone who appreciates fine design, I'm impressed by the curation here. The website is smooth, and the product photography makes it easy to choose the right piece." },
  { name: 'Meenakshi S.', role: 'Verified Buyer', avatar: 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=150&h=150&fit=crop&crop=faces', text: "Gugan Furniture has a lovely selection of decor and storage pieces. I found the perfect TV unit for my hall, and customer support was very helpful with my queries." },
  { name: 'Vignesh P.', role: 'Tech Lead', avatar: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150&h=150&fit=crop&crop=faces', text: "The shopping experience is top-notch. I love the minimalist UI and how easy it is to compare furniture for different rooms. Checkout was smooth and the products feel solid." },
];
...
const getHeroSlides = (hero = {}) => {
  ...
  return [...new Set([...configuredSlides, ...configuredImages, ...primaryImage, ...DEFAULT_HERO_SLIDES])]
    .filter((image) => !isLegacyHeroImage(image));
};
...
const initHomeVideo = () => {
  ...
  const video = section?.querySelector('#home-furniture-video');
  ...
  const syncState = () => {
    ...
    toggle.setAttribute('aria-label', paused ? 'Play home furniture video' : 'Pause home furniture video');
...
const renderAuthLoading = () => {
  ...
  main.innerHTML = `
    ...
    sublabel: 'Opening your Gugan Furniture account',
    variant: 'compact',
  })}
      </div>
    </section>
    ...
    <section class="auth-page">
      <div class="auth-page-card">
        <div class="auth-header">
          <span class="auth-badge">GUGAN FURNITURE</span>
          <h2>${isSignup ? 'Create Your Account' : 'Welcome Back'}</h2>
          <p>${isSignup ? 'Join our home design community.' : 'Log in to your furniture shopping hub.'}</p>
        </div>
      ...
      : 'New to Gugan Furniture? <a href="/signup" id="switch-auth-route">Join now</a>'}
        </p>
      </div>
    </section>
  `;
  ...
  main.innerHTML = `
    <div class="container py-xl">
      ...
      <div class="product-grid" id="wishlist-grid">
        ${LoadingState({ label: 'Loading wishlist...', sublabel: 'Finding your saved furniture', variant: 'grid' })}
      </div>
    </div>
  `;
    ...
    <div class="container">
      <div class="footer-detailed-cols">
        <div class="footer-col-links">
          <h4>ONLINE FURNITURE SHOPPING</h4>
          <ul>
        ...
        <div class="footer-col-trust">
          <div class="trust-item">
            ...
            <p><strong>100% ORIGINAL</strong> guarantee for all furniture at gugan.com</p>
          </div>
          <div class="trust-item">
      ...
      <div class="footer-popular-searches">
        ...
        <p>
          <a href="#">Sofas</a> | <a href="#">Recliners</a> | <a href="#">Coffee Tables</a> | <a href="#">Dining Tables</a> | <a href="#">Queen Beds</a> | <a href="#">Wardrobes</a> | <a href="#">TV Units</a> | <a href="#">Bookshelves</a> | <a href="#">Office Chairs</a> | <a href="#">Study Desks</a> | <a href="#">Shoe Racks</a> | <a href="#">Outdoor Seating</a> | <a href="#">Mattresses</a> | <a href="#">Bedside Tables</a> | <a href="#">Wall Shelves</a> | <a href="#">Floor Lamps</a> | <a href="#">Pendant Lights</a> | <a href="#">Mirrors</a> | <a href="#">Rugs</a> | <a href="#">Cushions</a> | <a href="#">Storage Cabinets</a> | <a href="#">Dining Chairs</a> | <a href="#">Console Tables</a> | <a href="#">Bar Cabinets</a> | <a href="#">Home Decor</a> | <a href="#">Wooden Furniture</a> | <a href="#">Modular Furniture</a>
        </p>
      </div>

      <div class="footer-seo-block">
        <p><strong>GUGAN FURNITURE: THE ULTIMATE HOME FURNISHING DESTINATION</strong></p>
        <p>Gugan Furniture is a destination for furniture and home living, with a wide range of sofas, beds, dining sets, storage units, decor, lighting and more. It is time to redefine your rooms with carefully selected pieces that balance comfort, durability and style. You can shop online at Gugan from the comfort of your home and get your favourites delivered right to your doorstep.</p>
        <p><strong>HISTORY OF GUGAN</strong></p>
        <p>Becoming a trusted furniture destination takes sincere effort, digital convenience and a team focused on reliable service. Gugan began with a love for practical home design, and today it serves rooms of every kind with living room, bedroom, dining, office, storage, decor and lighting collections.</p>
        <p><strong>SHOP ONLINE AT GUGAN WITH COMPLETE CONVENIENCE</strong></p>
        <p>Another reason why Gugan works well as an online furniture store is the complete convenience it offers. You can compare designs, prices, materials and room categories in one place. A user-friendly interface guides your selection process, while product information and high-resolution images help you make confident buying decisions. You also have the freedom to choose your payment options, be it card or cash-on-delivery. The 14-day returns policy gives you more confidence as a buyer.</p>
      </div>
  ...
  header.innerHTML = `
    <nav class="container nav-wrapper">
      ...
      <a href="/" class="logo" id="logo-link">
        <img src="${logoUrl}" alt="Gugan Furniture">
      </a>
      <ul class="nav-links">
      ...
      <div class="nav-right">
        <div class="search-bar">
          <input type="text" id="nav-search-input" placeholder="Search for furniture, rooms and more">
        </div>
...
const renderAdmin = async (path = '/admin') => {
  ...
  content.innerHTML = LoadingState({ label: 'Loading admin...', sublabel: 'Getting your store controls ready', variant: 'compact' });
...
const adminProductForm = async (container, product) => {
  ...
  const SIZES = ['Standard', 'Compact', 'Large', 'Premium', 'Walnut', 'Oak'];
  ...
  try {
    ...
    catItems = (config.categories?.items || []).map(c => normalizeCategoryName(c.name)).filter(Boolean);
  } catch (_) { }
  const allCats = [...new Set([...CATEGORIES, ...catItems])];
  const selectedSizes = product?.sizes || ['Standard', 'Large', 'Premium'];
      ...
      <form id="product-form" class="admin-form" autocomplete="off">
        <div class="form-grid-2">
          <div class="form-group">
            ...
            <input type="text" name="name" value="${escapeAttr(product?.name || '')}" placeholder="e.g. Premium Sofa Set" required>
          </div>
          <div class="form-group">
            <label>Brand</label>
        ...
        <form id="cat-form" class="admin-form">
          <div class="form-grid-2">
            <div class="form-group">
              ...
              <input type="text" name="name" value="${escapeAttr(cat?.name || '')}" placeholder="e.g. LIVING ROOM" required>
            </div>
            <div class="form-group">
              <label>Tagline</label>
            ...
            <div class="form-group" style="grid-column:span 2">
              <label>Link</label>
              <input type="text" name="link" value="${escapeAttr(cat?.link || '')}" placeholder="/shop?category=Living%20Room">
            </div>
          </div>
          <div class="form-group">
      ...
      <form id="hero-form" class="admin-form">
        ...
        <div class="form-grid-2">
          <div class="form-group">
            ...
            <input type="text" name="headline" value="${escapeAttr(hero.headline || '')}" placeholder="COMFORTABLE HOMES START HERE">
          </div>
          <div class="form-group">
            <label>Subtext</label>
            <input type="text" name="subtext" value="${escapeAttr(hero.subtext || '')}" placeholder="Room-ready furniture for every corner">
          </div>
          <div class="form-group">
...
const renderShop = async () => {
  ...
  currentFilters.category = normalizeCategoryName(urlParams.get('category')) || null;
  currentFilters.search = urlParams.get('search') || '';
  currentFilters.brand = urlParams.get('brand') || null;

  setPageMeta({
    title: currentFilters.category ? `${currentFilters.category} Furniture | Gugan Furniture` : 'Shop Furniture Online | Gugan Furniture',
    description: currentFilters.search
      ? `Search results for ${currentFilters.search} at Gugan Furniture. Discover room-ready furniture, decor and lighting.`
      : 'Shop sofas, beds, dining sets, office desks, storage, decor and lighting at Gugan Furniture.',
  });
      ...
      <div class="container py-xl shop-layout">
        <aside class="shop-sidebar">
          ...
          <div class="filter-section">
            <h4>BRAND</h4>
            <div class="filter-options">
              ${['GUGAN', 'CasaWood', 'SleepCraft', 'WorkPro'].map(brand => `
                <label class="filter-checkbox">
          ...
          <div class="filter-section">
            ...
            <div class="price-range-inputs">
              <input type="range" min="0" max="50000" step="1000" value="${currentFilters.priceRange.max}" oninput="this.nextElementSibling.textContent = 'Under Rs. ' + Number(this.value).toLocaleString(); app.updateShopFilter('priceMax', this.value)">
        ...
        <div class="shop-content">
          ...
          <div class="product-grid mt-md" id="shop-product-grid">
            ${LoadingState({ label: 'Loading products...', sublabel: 'Arranging furniture for this collection', variant: 'grid' })}
          </div>
  ...
  window.app.clearFilters = () => {
    currentFilters = { ...currentFilters, category: null, brand: null, fabric: null, sizes: [], priceRange: { min: 0, max: 50000 }, search: '', offset: 0 };
    renderShop();
  };
...
const fetchShopProducts = async (append = false) => {
  ...
  if (!append) grid.innerHTML = LoadingState({ label: 'Loading products...', sublabel: 'Arranging furniture for this collection', variant: 'grid' });
...
const renderProductDetail = async (id) => {
  const main = document.getElementById('router-view');
  main.innerHTML = LoadingState({ label: 'Loading product...', sublabel: 'Setting up this furniture detail', variant: 'page' });
  try {
    let product;
    ...
    const sizes = Array.isArray(product.sizes) && product.sizes.length ? product.sizes : ['Standard', 'Premium'];
    const price = Number(product.price) || 0;
    const discount = Number(product.discount) || 0;
    ...
    const safeCategory = escapeHtml(product.category || 'Furniture');
    const safeMaterial = escapeHtml(product.fabric || 'Premium Finish');
    const safeProductDescription = escapeHtml(product.description || 'Experience lasting comfort and refined home style with this premium piece from Gugan Furniture.');
    const relatedData = await productService.getProducts({
      category: product.category,
      sort: 'created_at:desc',
      limit: 8,
    });
    let relatedProducts = (relatedData.products || [])
      .filter((item) => String(item.id) !== String(product.id))
      .slice(0, 4);

    if (relatedProducts.length < 4) {
      const fallbackData = await productService.getProducts({ sort: 'created_at:desc', limit: 8 });
      const existingIds = new Set(relatedProducts.map((item) => String(item.id)));
      relatedProducts = [
        ...relatedProducts,
        ...(fallbackData.products || []).filter((item) => String(item.id) !== String(product.id) && !existingIds.has(String(item.id))),
      ].slice(0, 4);
    }

    setPageMeta({
      title: `${product.name || 'Furniture'} | Gugan Furniture`,
      description: `${product.name || 'Furniture'} by ${product.brand || 'Gugan Furniture'} for ${product.category || 'your home'}. Shop online at Rs. ${finalPrice.toLocaleString('en-IN')}.`,
      image: displayImage,
    });
    setProductStructuredData(product, displayImage, finalPrice);

    main.innerHTML = `
      <article class="pdp-page">
        <nav class="pdp-breadcrumbs" aria-label="Breadcrumb">
          <button type="button" onclick="app.navigate('/')">Home</button>
          <span>/</span>
          <button type="button" onclick="app.navigate('/shop?category=${encodeURIComponent(product.category || '')}')">${safeCategory}</button>
          <span>/</span>
        ...
        <section class="related-products-section" aria-labelledby="related-products-title">
          <div class="section-header">
            <div>
              ...
              <h2 id="related-products-title">Complete This Room</h2>
            </div>
            <button class="view-all-btn" onclick="app.navigate('/shop?category=${encodeURIComponent(product.category || '')}')">VIEW ALL</button>
          </div>
          <div class="product-grid related-product-grid">
            ${relatedProducts.length ? relatedProducts.map(p => ProductCard(p)).join('') : '<p class="empty-state">More matching furniture is coming soon.</p>'}
          </div>
        </section>
      </article>
    `;
    ...
    document.querySelectorAll('.related-product-grid .product-card').forEach(card => {
      card.onclick = () => navigate(`/product/${card.dataset.id}`);
    });
    document.querySelectorAll('.pdp-related-thumb').forEach(thumb => {
      thumb.onclick = () => navigate(`/product/${thumb.dataset.id}`);
    });
  } catch (err) {
...
const renderHome = async () => {
  ...
  clearProductStructuredData();
  setPageMeta({
    title: 'Gugan Furniture | Furniture Store',
    description: 'Discover room-ready sofas, beds, dining sets, wardrobes, office furniture, decor and lighting at Gugan Furniture.',
  });
  ...
  const heroImageFallback = heroSlides[0] || DEFAULT_HERO_SLIDES[0];
  ...
  const heroHeadline = furnitureHeroText(hero.headline, 'COMFORTABLE HOMES START HERE');
  const heroSubtext = furnitureHeroText(hero.subtext, 'Room-ready furniture for every corner');
  const heroButtonText = furnitureHeroText(hero.buttonText, 'SHOP NOW');
    ...
    <div class="home-page">
      <section class="m-hero-card" id="hero-cta" style="cursor:pointer">
        ...
        <div class="m-hero-overlay">
          <span class="m-hero-badge">NEW HOME COLLECTION 2026</span>
          <h1>${escapeHtml(heroHeadline)}</h1>
          <p>${escapeHtml(heroSubtext)}</p>
          <button class="btn-gold">${escapeHtml(heroButtonText)}</button>
        </div>
      ...
      <section class="categories-section">
        <div class="cat-header container" style="margin-bottom: 8px;">
          <h2>Gugan Furniture Collection</h2>
          ...
          <div class="circle-track">
            ${[0, 1].map((copyIndex) => `
              <div class="circle-grid" ${copyIndex === 1 ? 'aria-hidden="true"' : ''}>
                ${circleItems.map((cat) => `
                  <button type="button" class="circle-item" data-link="${escapeAttr(safeAppLink(cat.link, '/shop'))}">
      ...
      const specialCategories = ['Outdoor', 'Storage', 'Decor', 'Lighting'];
      ...
      return `
          ...
          : `
                      ...
                      <h3>${escapeHtml(category)} Furniture</h3>
                    `
        }
                </div>
              ...
              <div class="category-showcase-rail ${specialCategories.includes(category) ? 'm-modern-grid-layout' : ''}" id="showcase-${categoryDomId(category)}">
                <div class="showcase-loading">Loading ${escapeHtml(category)} furniture...</div>
              </div>
      ...
      <section class="immersive-video-section">
        ...
        <div class="immersive-video-content container" style="justify-content: flex-end; padding-left: 30px; padding-right: 30px;">
          <div class="video-action-right">
            <button class="play-button-wrapper" id="home-play-btn" type="button" aria-label="Pause home furniture video" aria-pressed="true">
      ...
      <section class="home-products-section py-lg container">
        ...
        <div class="product-grid m-modern-grid-layout mt-md" id="best-sellers-list">${LoadingState({ label: 'Loading best sellers...', sublabel: 'Curating popular furniture picks', variant: 'grid' })}</div>
      </section>
    </div>
  `;
...
const renderHomeSections = async () => {
  const containers = {
    ...
    livingRoom: document.getElementById('showcase-living-room'),
    bedroom: document.getElementById('showcase-bedroom'),
    dining: document.getElementById('showcase-dining'),
    office: document.getElementById('showcase-office'),
    outdoor: document.getElementById('showcase-outdoor'),
    storage: document.getElementById('showcase-storage'),
    decor: document.getElementById('showcase-decor'),
    lighting: document.getElementById('showcase-lighting'),
  };
  ...
  try {
    ...
    const [newData, trendingData, livingRoomData, bedroomData, diningData, officeData, outdoorData, storageData, decorData, lightingData] = await Promise.all([
      productService.getProducts({ limit: 4, sort: 'created_at:desc' }), // logic for new
      productService.getProducts({ isTrending: true, limit: 4 }),
      productService.getProducts({ category: 'Living Room', limit: 4 }),
      productService.getProducts({ category: 'Bedroom', limit: 4 }),
      productService.getProducts({ category: 'Dining', limit: 4 }),
      productService.getProducts({ category: 'Office', limit: 4 }),
      productService.getProducts({ category: 'Outdoor', limit: 4 }),
      productService.getProducts({ category: 'Storage', limit: 4 }),
      productService.getProducts({ category: 'Decor', limit: 4 }),
      productService.getProducts({ category: 'Lighting', limit: 4 }),
    ]);

    if (containers.best) {
      containers.best.innerHTML = bestData.products.length ? bestData.products.map(p => ProductCard(p)).join('') : '<p>More furniture coming soon.</p>';
    }
    if (containers.new) {
    ...
    const renderSubcatCards = (items, sprite) => items.map((cat, index) => `
      <article class="m-subcat-card" onclick="app.navigate('/shop?search=${encodeURIComponent(cat.name)}')">
        ${cat.image
          ? `<img class="m-subcat-card-media" src="${escapeAttr(cat.image)}" alt="${escapeAttr(cat.name)}" data-fallback-src="${escapeAttr(defaultImageFallback)}">`
          : `<div
    ...
    if (containers.livingRoom) {
      const livingRoomSubcats = [
        { name: 'Sofas', image: 'https://images.unsplash.com/photo-1555041469-a586c61ea9bc?q=80&w=700&auto=format&fit=crop' },
        { name: 'Recliners', image: 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?q=80&w=700&auto=format&fit=crop' },
        { name: 'Coffee Tables', image: 'https://images.unsplash.com/photo-1499933374294-4584851497cc?q=80&w=700&auto=format&fit=crop' },
        { name: 'TV Units', image: 'https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?q=80&w=700&auto=format&fit=crop' }
      ];
      containers.livingRoom.innerHTML = renderSubcatCards(livingRoomSubcats, '/category-cards/men-categories.png');
    }
    if (containers.bedroom) {
      const bedroomSubcats = [
        { name: 'Beds', image: 'https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?q=80&w=700&auto=format&fit=crop' },
        { name: 'Mattresses', image: 'https://images.unsplash.com/photo-1631049307264-da0ec9d70304?q=80&w=700&auto=format&fit=crop' },
        { name: 'Wardrobes', image: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?q=80&w=700&auto=format&fit=crop' },
        { name: 'Bedside Tables', image: 'https://images.unsplash.com/photo-1595526114035-0d45ed16cfbf?q=80&w=700&auto=format&fit=crop' }
      ];
      containers.bedroom.innerHTML = renderSubcatCards(bedroomSubcats, '/category-cards/women-categories.png');
    }
    if (containers.dining) {
      const diningSubcats = [
        { name: 'Dining Tables', image: 'https://images.unsplash.com/photo-1617806118233-18e1de247200?q=80&w=700&auto=format&fit=crop' },
        { name: 'Dining Chairs', image: 'https://images.unsplash.com/photo-1604578762246-41134e37f9cc?q=80&w=700&auto=format&fit=crop' },
        { name: 'Bar Cabinets', image: 'https://images.unsplash.com/photo-1600566753190-17f0baa2a6c3?q=80&w=700&auto=format&fit=crop' },
        { name: 'Crockery Units', image: 'https://images.unsplash.com/photo-1618220179428-22790b461013?q=80&w=700&auto=format&fit=crop' }
      ];
      containers.dining.innerHTML = renderSubcatCards(diningSubcats, '/category-cards/boys-categories.png');
    }
    if (containers.office) {
      const officeSubcats = [
        { name: 'Study Desks', image: 'https://images.unsplash.com/photo-1518455027359-f3f8164ba6bd?q=80&w=700&auto=format&fit=crop' },
        { name: 'Office Chairs', image: 'https://images.unsplash.com/photo-1580480055273-228ff5388ef8?q=80&w=700&auto=format&fit=crop' },
        { name: 'Bookshelves', image: 'https://images.unsplash.com/photo-1524758631624-e2822e304c36?q=80&w=700&auto=format&fit=crop' },
        { name: 'Workstations', image: 'https://images.unsplash.com/photo-1497366754035-f200968a6e72?q=80&w=700&auto=format&fit=crop' }
      ];
      containers.office.innerHTML = renderSubcatCards(officeSubcats, '/category-cards/girls-categories.png');
    }
    if (containers.outdoor) {
      const outdoorSubcats = [
        { name: 'Patio Sets', image: 'https://images.unsplash.com/photo-1600210492493-0946911123ea?q=80&w=600&auto=format&fit=crop' },
        { name: 'Garden Chairs', image: 'https://images.unsplash.com/photo-1595526114035-0d45ed16cfbf?q=80&w=600&auto=format&fit=crop' },
        { name: 'Balcony Tables', image: 'https://images.unsplash.com/photo-1600566753190-17f0baa2a6c3?q=80&w=600&auto=format&fit=crop' },
        { name: 'Outdoor Loungers', image: 'https://images.unsplash.com/photo-1617104678098-de229db51175?q=80&w=600&auto=format&fit=crop' },
        { name: 'Swing Chairs', image: 'https://images.unsplash.com/photo-1598300042247-d088f8ab3a91?q=80&w=600&auto=format&fit=crop' }
      ];
      containers.outdoor.innerHTML = outdoorSubcats.map((cat) => `
    ...
    if (containers.storage) {
      const storageSubcats = [
        { name: 'Wardrobes', image: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?q=80&w=700&auto=format&fit=crop' },
        { name: 'Shoe Racks', image: 'https://images.unsplash.com/photo-1595526114035-0d45ed16cfbf?q=80&w=700&auto=format&fit=crop' },
        { name: 'Cabinets', image: 'https://images.unsplash.com/photo-1618220179428-22790b461013?q=80&w=700&auto=format&fit=crop' },
        { name: 'Wall Shelves', image: 'https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?q=80&w=700&auto=format&fit=crop' },
        { name: 'Bookcases', image: 'https://images.unsplash.com/photo-1524758631624-e2822e304c36?q=80&w=600&auto=format&fit=crop' }
      ];
      containers.storage.innerHTML = storageSubcats.map((cat, index) => `
        <div class="m-modern-cat-card" onclick="app.navigate('/shop?search=${encodeURIComponent(cat.name)}')">
          <div class="m-modern-cat-card-img">
            ${cat.image
          ? `<img src="${cat.image}" alt="${cat.name}">`
    ...
    if (containers.decor) {
      const decorSubcats = [
        { name: 'Mirrors', image: 'https://images.unsplash.com/photo-1513519245088-0e12902e5a38?q=80&w=700&auto=format&fit=crop' },
        { name: 'Rugs', image: 'https://images.unsplash.com/photo-1519710164239-da123dc03ef4?q=80&w=700&auto=format&fit=crop' },
        { name: 'Wall Art', image: 'https://images.unsplash.com/photo-1513519245088-0e12902e5a38?q=80&w=700&auto=format&fit=crop' },
        { name: 'Cushions', image: 'https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?q=80&w=700&auto=format&fit=crop' },
        { name: 'Planters', image: 'https://images.unsplash.com/photo-1485955900006-10f4d324d411?q=80&w=600&auto=format&fit=crop' }
      ];
      containers.decor.innerHTML = decorSubcats.map((cat, index) => `
        <div class="m-modern-cat-card" onclick="app.navigate('/shop?search=${encodeURIComponent(cat.name)}')">
          <div class="m-modern-cat-card-img">
            ${cat.image
          ? `<img src="${cat.image}" alt="${cat.name}">`
    ...
    if (containers.lighting) {
      const lightingSubcats = [
        { name: 'Floor Lamps', image: 'https://images.unsplash.com/photo-1507473885765-e6ed057f782c?q=80&w=700&auto=format&fit=crop' },
        { name: 'Table Lamps', image: 'https://images.unsplash.com/photo-1513506003901-1e6a229e2d15?q=80&w=700&auto=format&fit=crop' },
        { name: 'Pendant Lights', image: 'https://images.unsplash.com/photo-1524484485831-a92ffc0de03f?q=80&w=700&auto=format&fit=crop' },
        { name: 'Wall Lights', image: 'https://images.unsplash.com/photo-1565814329452-e1efa11c5b89?q=80&w=700&auto=format&fit=crop' },
        { name: 'Chandeliers', image: 'https://images.unsplash.com/photo-1513506003901-1e6a229e2d15?q=80&w=600&auto=format&fit=crop' }
      ];
      containers.lighting.innerHTML = lightingSubcats.map((cat, index) => `
        <div class="m-modern-cat-card" onclick="app.navigate('/shop?search=${encodeURIComponent(cat.name)}')">
          <div class="m-modern-cat-card-img">
            ${cat.image
          ? `<img src="${cat.image}" alt="${cat.name}">`
